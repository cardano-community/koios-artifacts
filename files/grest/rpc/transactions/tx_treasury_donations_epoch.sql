CREATE OR REPLACE FUNCTION grest.tx_treasury_donations_epoch(_epoch_no numeric DEFAULT NULL)
RETURNS TABLE (
  epoch_no word31type,
  tx_hash text,
  block_hash text,
...
AS $$
  ...
  WHERE b.epoch_no = COALESCE(_epoch_no, (SELECT MAX(epoch_param.epoch_no) FROM public.epoch_param))::word31type
