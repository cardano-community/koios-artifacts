CREATE TABLE IF NOT EXISTS grest.pool_info_cache_v2 (
...
COMMENT ON TABLE grest.pool_info_cache_v2 IS 'A summary of all pool parameters and updates';
...
BEGIN
  ...
  SELECT MAX(epoch_no) FROM public.epoch into _current_epoch_no;
  ...
  WITH
    latest_pool_updates AS (
      ...
      ORDER BY hash_id, cert_index DESC
    )

    INSERT INTO grest.pool_info_cache_v2 (
      tx_id,
      update_id,
      tx_hash,
      block_time,
