...
AS $$
  ...
  FROM pool_hash AS ph
    ...
    -- or current epoch if there haven't been further updates
      AND es.epoch_no BETWEEN pu.active_epoch_no AND
      (SELECT COALESCE(MIN(pu3.active_epoch_no) - 1, (SELECT MAX(no) FROM epoch)) FROM pool_update pu3 WHERE pu3.hash_id = pu.hash_id AND pu3.id > pu.id)
