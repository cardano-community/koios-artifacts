...
BEGIN
  ...
  SELECT
    ...
    END
    INTO gap_id, gap_index, gap_type, gap_tx_hash, gap_enacted_tx_id
  FROM public.gov_action_proposal AS gap
    INNER JOIN tx ON tx.id = gap.tx_id
  WHERE gap.type = any(array['NewCommittee','NoConfidence']::govactiontype[])
    AND gap.enacted_epoch IS NOT NULL
  ORDER BY gap.enacted_epoch DESC
  LIMIT 1;

  IF gap_type = 'NoConfidence' THEN
    RETURN QUERY (
      SELECT
        (SELECT grest.cip129_to_gov_action_id(gap_tx_hash, gap_index)),
        ENCODE(gap_tx_hash, 'hex'),
        gap_index,
        NULL::bigint,
        NULL::bigint,
        NULL::jsonb
  ...
  ELSE
    RETURN QUERY (
      SELECT
        CASE
          WHEN gap_id IS NULL THEN NULL
          ELSE (SELECT grest.cip129_to_gov_action_id(gap_tx_hash, gap_index))
        END,
        CASE
          WHEN gap_id IS NULL THEN NULL
          ELSE ENCODE(gap_tx_hash, 'hex')
        END,
        CASE
          WHEN gap_id IS NULL THEN NULL
          ELSE gap_index
